...
export const description = "Respiratory rate chart for last 7 records"
  ...
  useEffect(() => {
    const fetchRespiratoryRateData = async () => {
      try {
        ...
        let query = supabase
          .from("vitals_data")
          .select("respiratory_rate, timestamp")
          .eq("user_id", userId)
      ...
      <CardFooter className="pt-0">
        {average !== null && (
          <div className="flex w-full items-center justify-between text-sm">
            ...
            <span className="font-medium">{average} breaths/min</span>
          </div>
        )}
      </CardFooter>
    </Card>
